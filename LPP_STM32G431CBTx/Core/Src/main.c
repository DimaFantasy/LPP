...
int main(void)
{
  ...
  // 2: DMA лазера — DMA1_Ch1 (ON) и DMA1_Ch4 (OFF)
  HAL_NVIC_SetPriority(DMA1_Channel1_IRQn, 2, 0);
  HAL_NVIC_EnableIRQ(DMA1_Channel1_IRQn);
  HAL_NVIC_SetPriority(DMA1_Channel4_IRQn, 2, 0);
  HAL_NVIC_EnableIRQ(DMA1_Channel4_IRQn);

  // 3: DMA2_CH1 (M2M копирование данных)
...
/**
  ...
  * G431: HSE 4MHz, PLL → 170MHz, HSI48 для USB
  */
void SystemClock_Config(void)
{
...
/**
  ...
  * G431 @ 170MHz: Prescaler=168 → 170MHz/169 ≈ 1МГц тик
  */
static void MX_TIM3_Init(void)
{
...
// ============================================================================
// СЕКЦИЯ: МОТОР ОСИ X (TIM2) — STM32G431 @ 170МГц
// ============================================================================
...
void XMotorInit(X_MOTOR_MODE_T mode) {
    ...
    if (mode == X_MOTOR_MODE_STEP) {
        htim2.Init.Prescaler = 169; // 170МГц / 170 = 1МГц → 1 тик = 1 мкс
...
// ============================================================================
// СЕКЦИЯ: ЛАЗЕР (TIM1 + DMA1 + DMAMUX1)
// ============================================================================
// LASER: Включение по TIM1_UP → DMA1_Ch1, выключение по TIM1_CH1 → DMA1_Ch4.
...
/**
 * @brief Привязка DMA каналов к лазеру
 * G431 (DMA1 + DMAMUX1):
 *   Laser ON  = DMA1_Ch1, DMAMUX REQ = TIM1_UP
 *   Laser OFF = DMA1_Ch4, DMAMUX REQ = TIM1_CH1
 */
void PWM_LaserLight_LinkHardware(void) {
    ...
    DMA1->IFCR = DMA_IFCR_CGIF1 | DMA_IFCR_CGIF4;

    if (!laser->used || laser->bsrr == (uint32_t*)&GPIO_NULL_REG) return;

    raw_pin_masks[0] = laser->pin;

    uint32_t ccr_base = DMA_CCR_DIR | DMA_CCR_CIRC |
                        (0x02 << DMA_CCR_PSIZE_Pos) | (0x02 << DMA_CCR_MSIZE_Pos) |
                        LL_DMA_PRIORITY_VERYHIGH;

    // Laser ON — DMA1_Ch1, DMAMUX REQ = TIM1_UP
    LL_DMAMUX_SetRequestID(DMAMUX1, 0, LL_DMAMUX_REQ_TIM1_UP);
    DMA1_Channel1->CPAR  = (uint32_t)laser->bsrr;
    DMA1_Channel1->CMAR  = (uint32_t)&dma_pwm_masks[0][0];
    DMA1_Channel1->CNDTR = 1;
    DMA1_Channel1->CCR   = ccr_base | DMA_CCR_EN;

    // Laser OFF — DMA1_Ch4, DMAMUX REQ = TIM1_CH1
    LL_DMAMUX_SetRequestID(DMAMUX1, 3, LL_DMAMUX_REQ_TIM1_CH1);
    DMA1_Channel4->CPAR  = (uint32_t)laser->bsrr;
    DMA1_Channel4->CMAR  = (uint32_t)&dma_pwm_masks[0][1];
...
void SetLaserLightPWMFrequency(uint32_t freq_hz) {
    ...
    TIM1->CCMR1 = 0;
    TIM1->CCMR2 = 0;
    TIM1->DIER  = TIM_DIER_UDE |   // Update → DMA1_Ch1 (Laser ON)
                  TIM_DIER_CC1DE;  // CCR1   → DMA1_Ch4 (Laser OFF)
...
// ============================================================================
// СЕКЦИЯ: DMA M2M (DMA2_Channel1)
// G431: DMA2 + DMAMUX1, Channel 1
// ============================================================================
...
void FIFO_DATA_StartDma(uint32_t src, uint32_t dst, uint16_t count) {
    ...
    DMA2_Channel1->CPAR  = src;
    DMA2_Channel1->CMAR  = dst;
    DMA2_Channel1->CNDTR = count;

    DMA2_Channel1->CCR = DMA_CCR_MEM2MEM | DMA_CCR_MINC | DMA_CCR_PINC |
...
void EXTI9_5_IRQHandler(void) {
    ...
    if (pr) { EXTI->PR1 = pr; LPP_GPIO_EXTI_Handler(pr); }
}
